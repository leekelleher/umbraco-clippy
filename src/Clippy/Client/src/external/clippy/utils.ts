...
export function getWidth(el: HTMLElement, type: 'inner' | 'outer' | 'width' | 'full') {
	...
	if (type === 'width')
		// .width()
		return (
			el.clientWidth - parseInt(s.getPropertyValue('padding-left')) - parseInt(s.getPropertyValue('padding-right'))
		);
	else if (type === 'full')
...
export function getHeight(el: HTMLElement, type: 'inner' | 'outer' | 'height' | 'full') {
	...
	if (type === 'height')
		// .height()
		return (
			el.clientHeight - parseInt(s.getPropertyValue('padding-top')) - parseInt(s.getPropertyValue('padding-bottom'))
		);
	else if (type === 'full')
		// .outerWidth(includeMargins = true)
